...
function buildSlidesData(texts) {
  return slidesBlueprint.map((slide) => {
    ...
    if (slide.type === 'service') {
      ...
      return {
        ...slide,
        title: t(texts, `service${n}Name`), text: t(texts, `service${n}Text`), inputLabel: t(texts, 'serviceInputLabel'),
...
function bindInquiryForm(texts) { const form = document.getElementById('inquiryForm'); const feedback = document.getElementById('formFeedback'); if (!form || !feedback) return; form.addEventListener('submit', (event) => { event.preventDefault(); const name = form.elements.name.value.trim(); const email = form.elements.email.value.trim(); const service = form.elements.service.value.trim(); const message = form.elements.message.value.trim(); const validEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email); if (!name || !validEmail || !service || !message) { feedback.textContent = t(texts, 'formError'); feedback.classList.remove('is-success'); return; } feedback.textContent = t(texts, 'contactSuccess'); feedback.classList.add('is-success'); form.reset(); }); }
...
function updateCounter() { const current = String(activeIndex + 1).padStart(2, '0'); const total = String(slidesBlueprint.length).padStart(2, '0'); document.getElementById('slideCounter').textContent = `${current} / ${total}`; }
