...
function renderNav(texts) { const nav = document.getElementById('topNav'); const items = [{ k: 'navOverview', index: 0 }, { k: 'navExteriorGroup', index: 1 }, { k: 'navInteriorGroup', index: 2, group: 'interior' }, { k: 'navStagingGroup', index: 5, group: 'staging' }, { k: 'navInquiry', index: 8 }]; nav.innerHTML = `${items.map((item) => `<button class=\"nav-btn\" data-index=\"${item.index}\" data-group=\"${item.group || ''}\">${t(texts, item.k, item.k)}</button>`).join('')}<div class=\"nav-submenu\" id=\"navSubmenu\" aria-hidden=\"true\"></div>`; const submenu = nav.querySelector('#navSubmenu'); const submenuMap = { interior: [2, 3, 4], staging: [5, 6, 7] }; nav.querySelectorAll('.nav-btn').forEach((b) => b.addEventListener('click', () => { const group = b.dataset.group; if (group && submenuMap[group]) { submenu.innerHTML = submenuMap[group].map((n) => `<button class=\"nav-submenu-btn\" type=\"button\" data-index=\"${n}\">${t(texts, `service${n}Name`, `Service ${n}`)}</button>`).join(''); submenu.classList.add('is-open'); submenu.setAttribute('aria-hidden', 'false'); submenu.querySelectorAll('.nav-submenu-btn').forEach((item) => item.addEventListener('click', () => goTo(Number(item.dataset.index)))); } else { submenu.classList.remove('is-open'); submenu.setAttribute('aria-hidden', 'true'); } goTo(Number(b.dataset.index)); })); }
